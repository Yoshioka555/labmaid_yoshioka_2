...
class _ChatRoomListPage extends State<ChatRoomListPage> {
  ...
  Future<void> getGroupChatRoomList() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (mounted) {
        setState(() {
          groupChatRoomList = body
              .map((dynamic json) => GetGroupChatRoomData.fromJson(json))
              .toList();
        });
      }
    } else {
  ...
  Future<void> fetchChatRoomList() async {
    ...
    if (responseGet.statusCode == 200) {
      ...
      if (mounted) {
        setState(() {
          userData = body
              .map((dynamic json) => UserPrivateChatData.fromJson(json))
              .toList();
        });
      }
    ...
    if (myData != null) {
      // 参加中のグループチャットの一覧を取得
      var uriEntryGroup =
          Uri.parse('${httpUrl}get_entry_group_chat_room/${myData!.id}');
      // GETリクエストを送信
      ...
      if (responseEntryGroup.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            groupChatData = body
                .map((dynamic json) => GroupChatRoomData.fromJson(json))
                .toList();
          });
        }
      } else {
      ...
      var uriNotEntryGroup =
          Uri.parse('${httpUrl}get_not_entry_group_chat_room/${myData!.id}');
      // GETリクエストを送信
  ...
  void _connectGroupWebSocket() {
    ...
    _channel.stream.listen((message) async {
      ...
      if (decodedMessage['type'] == 'broadcast') {
        ...
        // 対象メッセージを探し、isRead を更新
        final int index =
            groupChatData.indexWhere((group) => group.id == groupChatRoomId);
        if (index != -1) {
  ...
  Future<void> getGroupChatUserMyData(int groupChatRoomId) async {
    ...
    // MyUser情報を取得
    var uri = Uri.parse(
        '${httpUrl}get_group_chat_room_user/$groupChatRoomId/${currentUser!.uid}');
      ...
      child: Theme(
        data: ThemeData(
          ...
          colorScheme: ColorScheme.fromSwatch(
              primarySwatch: Colors.orange), //イベントページのテーマカラーを設定（purple）
        ),
        child: Scaffold(
          appBar: AppBar(
                    ...
                    onPressed: () async {
                      ...
                      await Navigator.of(context).push(
                        MaterialPageRoute(builder: (context) {
                          return GroupChatRoomDeletePage(
                              groupChatRoomList: groupChatRoomList);
                        }),
                      );
                    }),
              ),
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return GestureDetector(
                              onTap: () async {
                                //個人チャットルーム遷移
                                await createOrGetPrivateChatRoom(
                                    userData[index].id);
                                await updateUnreadPrivateMessage(
                                    privateChatroomId, userData[index].id);
                                await Navigator.of(context).push(
                                  MaterialPageRoute(builder: (context) {
                              ...
                              child: Container(
                                ...
                                child: ListTile(
                                    ...
                                    subtitle: userData[index].group ==
                                            'Network班'
                                        ? Text(
                                            'Net班　${userData[index].grade}　${userData[index].status}')
                                        : Text(
                                            '${userData[index].group}　${userData[index].grade}　${userData[index].status}'),
                                    trailing: Column(
                                        ...
                                        userData[index].updatedAt != null
                                            ? Builder(
                                                builder: (context) {
                                                  ...
                                                  String timeText;
                                                  int dayDifference = nowDate
                                                      .difference(updatedDate)
                                                      .inDays;
                                                  ...
                                                  if (dayDifference == 0) {
                                                    ...
                                                    // 1週間以内
                                                    timeText =
                                                        '${DateFormat.E('ja').format(updated)}曜日';
                                                  } else {
                                                    // それ以前
                                                    timeText =
                                                        DateFormat.yMd('ja')
                            ...
                            return GestureDetector(
                              onTap: () async {
                                ...
                                await getGroupChatUsers(
                                    groupChatData[index].id);
                                await getGroupChatUserMyData(
                                    groupChatData[index].id);
                                await updateUnreadGroupMessage(
                                    groupChatData[index].id, myData!.id);
                                if (groupMyData != null) {
                                  await Navigator.of(context).push(
                                    MaterialPageRoute(builder: (context) {
                              ...
                              child: Container(
                                ...
                                child: ListTile(
                                  leading: CircleAvatar(
                                    backgroundColor: Colors.grey,
                                    radius: 50,
                                    backgroundImage: groupChatData[index]
                                                .imageURL !=
                                            ''
                                        ? Image.network(
                              ...
                              child: Container(
                                ...
                                child: ListTile(
                                  leading: CircleAvatar(
                                    backgroundColor: Colors.grey,
                                    radius: 50,
                                    backgroundImage: notGroupChatData[index]
                                                .imageURL !=
                                            ''
                                        ? Image.network(
  ...
  Future updateUnreadPrivateMessage(int roomId, String userId) async {
    var url =
        Uri.parse('${httpUrl}private_message_unread_update/$roomId/$userId');
  ...
  Future getGroupChatUsers(int groupChatRoomId) async {
    ...
    if (response.statusCode == 200) {
      ...
      // 必要なデータを取得
      groupChatUsers =
          body.map((dynamic json) => GroupChatUserData.fromJson(json)).toList();
    } else {
      // リクエストが失敗した場合の処理
      print('リクエストが失敗しました: ${response.statusCode}');
    }
  ...
  Future updateUnreadGroupMessage(int roomId, String userId) async {
    var url =
        Uri.parse('${httpUrl}group_message_unread_update/$roomId/$userId');
